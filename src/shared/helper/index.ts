...
export class TodoHelper {
  ...
  public static getTodos = (): TodoItem[] => {
    const savedTodosString = window.localStorage.getItem(TodoHelper.LOCAL_STORAGE_KEY);
  ...
  public static add = (todos: TodoItem[], todoToAdd: TodoItem): TodoItem[] => {
    const changedTodos = [
      ...
      {
        ...todoToAdd,
        id: TodoHelper._getNextId(todos),
      },
    ] as TodoItem[];
  ...
  public static toggle = (todos: TodoItem[], todoToToggle: TodoItem): TodoItem[] => {
  ...
  public static edit = (todos: TodoItem[], todoToEdit: TodoItem, value: string): TodoItem[] => {
    const changedTodos = todos.map((todo: TodoItem) =>
      todo.id === todoToEdit.id ? ({ ...todo, text: value } as TodoItem) : todo,
    );
    TodoHelper._save(changedTodos);
    return changedTodos;
  };

  public static delete = (todos: TodoItem[], todoToDelete: TodoItem): TodoItem[] => {
    const changedTodos = todos
  ...
  private static _getNextId = (todos: TodoItem[]): number => {
    return todos.length > 0 ? Math.max(...todos.map((todo: TodoItem) => todo.id)) + 1 : 1;
  };

  private static _save = (todos: TodoItem[]): void => {
    window.localStorage.removeItem(TodoHelper.LOCAL_STORAGE_KEY);
    window.localStorage.setItem(TodoHelper.LOCAL_STORAGE_KEY, JSON.stringify(todos));
  };
}
